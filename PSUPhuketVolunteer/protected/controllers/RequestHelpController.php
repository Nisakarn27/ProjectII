...
if ( isset( $_POST['RequestHelp'] ) )
{
	...
	if ( $news = CUploadedFile::getInstance ( $model,'RequestImage')) {
		// path for file upload
		$path = Yii::getPathOfAlias ( 'webroot' ) . '/upload/Request/';
		// use image name as Name Image
		$filename = $news ;
		// uploaded image to path
		$news->saveAs ( $path . $filename );
		$model->RequestImage = $filename;}
		...
		if ( $model->save() )
			$this->redirect( array( 'view', 'id'=>$model->RequestID ) );
}
